...
-- 2c Find all actors whose last names contain the letters `LI`. This time, order the rows by last name and first name, in 
-- that order:
select actor_ID, first_name, last_name from actor
...
from country
-- where country = 'Afghanistan'  'Bangladesh' Or 'China';
where country IN ('Afghanistan', 'Bangladesh', 'China');

-- * 3a. Add a `middle_name` column to the table `actor`. Position it between `first_name` and `last_name`. 
-- Hint: you will need to specify the data type.

-- select actor_id, first_name, 'middle_name' VARCHAR(30), last_name
-- from actor
ALTER TABLE actor
ADD COLUMN middle_name VARCHAR(45);

-- 3b. Change the data type of the `middle_name` column to `blobs`.
-- alter TABLE actor MODIFY middle_name.reset index blobs;
ALTER TABLE actor 
...
-- name after all! In a single query, if the first name of the actor is currently `HARPO`, change it to `GROUCHO`. Otherwise, 
